...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
box_plot1 <- ggplot(heart_data, aes(x = Sex, y = MaxHR, fill = factor(HeartDisease))) +
...
box_plot2 <- ggplot(heart_data, aes(x = Sex, y = Age, fill = factor(HeartDisease))) +
  geom_boxplot(alpha = 0.7) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
grouped_bar_plot<- ggplot(heart_data, aes(x = ChestPainType, fill = factor(HeartDisease))) +
